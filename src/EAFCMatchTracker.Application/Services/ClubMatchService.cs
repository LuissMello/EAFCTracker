...
public class ClubMatchService : IClubMatchService
{
    ...
    private async Task<Domain.Models.OverallStats?> FetchOverallStatsAsync(long clubId, CancellationToken ct)
    {
        var baseUrl = _config["EAFCSettings:BaseUrl"] ?? "";
    ...
    private async Task<List<Domain.Models.PlayoffAchievement>?> FetchPlayoffAchievementsAsync(long clubId, CancellationToken ct)
    {
        var baseUrl = _config["EAFCSettings:BaseUrl"] ?? "";
